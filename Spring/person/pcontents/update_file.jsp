...
<c:set var="contentsno" value="${pcontentsVO.pcontentsno }" />
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <span class='menu_divide' >│</span>
    <A href="./update_text.do?pcontentsno=${pcontentsno}">수정</A>
    <span class='menu_divide' >│</span>
    <A href="./update_text.do?pcontentsno=${pcontentsno}">파일 수정</A>  
  </ASIDE> 
  
    <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_by_pcateno_search_paging.do'>
      <input type='hidden' name='cateno' value='${pcateVO.pcateno }'>
      <input type='text' name='word' id='word' value='${param.word }' style='width: 20%;'>
      <button type='submit'>검색</button>
      <c:if test="${param.word.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_by_pcateno_search_paging.do?cateno=${pcateVO.pcateno}&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
    ...
    <ul>
      <li class="li_none">
        <DIV style='text-align: center; width: 50%; float: left;'>
          <c:set var="file1saved" value="${pcontentsVO.file1saved.toLowerCase() }" />
          <c:set var="thumb1" value="${pcontentsVO.thumb1 }" />
          <c:choose>
            <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
              <IMG src="/pcontents/storage/${file1saved }" style='width: 90%;'> 
            </c:when>
            <c:otherwise> <!-- 이미지가 없음 -->
               <IMG src="/pcontents/images/none1.png" style="width: 90%;"> 
            </c:otherwise>
          </c:choose>
        ...
        <DIV style='text-align: left; width: 47%; float: left;'>
          ...
          <FORM name='frm' method='POST' action='./update_file.do' enctype="multipart/form-data">
            <input type="hidden" name="pcontentsno" value="${pcontentsno }">
