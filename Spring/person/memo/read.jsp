...
<DIV class='title_line'>
  ...
  <A href="./list.do" class='title_link'>${memoVO.title }</A>
</DIV>
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <span class='menu_divide' >│</span>    >
    <A href="./update_text.do?&now_page=${param.now_page}">수정</A>
    <span class='menu_divide' >│</span>
    <A href="./update_file.do?&now_page=${param.now_page}">파일 수정</A>  
    <span class='menu_divide' >│</span>
    <A href="./delete.do?memono=${memono}&now_page=${param.now_page}">삭제</A>  
  </ASIDE> 
        ...
        <DIV style="width: 50%; float: left; margin-right: 10px;">
            <c:choose>
              <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
                <%-- /static/memo/storage/ --%>
                <IMG src="/memo/storage/${file1saved }" style="width: 100%;"> 
              </c:when>
              <c:otherwise> <!-- 기본 이미지 출력 -->
                <IMG src="/memo/images/none1.png" style="width: 100%;"> 
              </c:otherwise>
            </c:choose>
        </DIV>
      ...
      <li class="li_none">
        <DIV>
          <c:if test="${file1.trim().length() > 0 }">
            첨부 파일: <A href='/download?dir=/memo/storage&filename=${file1saved}&downname=${file1}'>${file1}</A> (${size1_label})  
          </c:if>
        </DIV>
      </li>   
    </ul>
