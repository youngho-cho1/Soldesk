...
<DIV class='title_line'>
  ...
  <A href="./list_by_pcategrpno.do?pcategrpno=${param.pcategrpno }" class='title_link'>${pcategrpVO.name }</A> > 
  ${pcateVO.name } 수정 
</DIV>
...
<DIV class='content_body'>
  ...
  <DIV id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    ...
    <FORM name='frm_delete' id='frm_delete' method='POST' action='./delete.do'>
      <input type='hidden' name='pcateno' id='pcateno' value="${pcateVO.pcateno }">
      
      <label>그룹 번호</label>: ${pcateVO.pcategrpno }  
      <label>카테고리</label>: ${pcateVO.name}  
