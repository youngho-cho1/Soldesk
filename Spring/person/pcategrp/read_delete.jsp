...
<DIV class='title_line'>카테고리 그룹 > ${pcategrpVO.name } 삭제</DIV>
...
<DIV class='content_body'>
  <DIV id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    ...
    <FORM name='frm_delete' id='frm_delete' method='POST' action='./delete.do'>
      ...
      <label>그룹 이름</label>: ${pcategrpVO.name }  
      <label>순서</label>: ${pcategrpVO.seqno }   
      <label>출력 형식</label>: ${pcategrpVO.visible }  
  ...
  <TABLE class='table table-striped'>
    ...
    <c:forEach var="categrpVO" items="${list}">
      <c:set var="pcategrpno" value="${categrpVO.pcategrpno }" />
