    ...
    <UL>
      <c:choose>
        <c:when test="${code == 'create_success'}"> <%-- mav.addObject("code", "create_success"); --%>
          <LI class='li_none'>
            <span class="span_success">새로운 카테고리 그룹 [${pcategrpVO.name }] 등록했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${code == 'create_fail'}"> <%-- mav.addObject("code", "create_fail");  --%>
          <LI class='li_none'>
            <span class="span_success">새로운 카테고리 그룹 [${pcategrpVO.name }] 등록에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        ...
        <c:when test="${code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">[${pcategrpVO.name }] 카테고리 그룹 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
        <c:when test="${code == 'delete_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">[${pcategrpVO.name }] 카테고리 그룹 삭제에 실패했습니다.</span>
          </LI>                                                                      
        </c:when> 
        <c:otherwise>
      ...
      <LI class='li_none'>
        ...
        <c:choose>
          <c:when test="${msg == 'child_record_found' }">
            ...
            카테고리를 모두 삭제해주세요.<br>
            『<A href="../pcate/list_by_pcategrpno.do?pcategrpno=${param.pcategrpno }">소속된 카테고리 삭제</A>』
          </c:when>
        </c:choose>
      </LI>
      <LI class='li_none'>
        ...
        <c:choose>
            ...
            <c:when test="${cnt > 0 }">
                <button type='button' onclick="location.href='./pcreate.do?pcategrpno=${pcategrpno}'" class="btn btn-primary">새로운 카테고리 등록</button>
            </c:when>
        </c:choose>
